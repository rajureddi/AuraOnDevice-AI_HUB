...
import com.aura_on_device_ai.mnnllm.android.llm.ChatSession
import com.aura_on_device_ai.mnnllm.android.R
import com.aura_on_device_ai.mnnllm.android.audio.AudioChunksPlayer
import com.aura_on_device_ai.mnnllm.android.benchmark.BenchmarkModule
import com.aura_on_device_ai.mnnllm.android.modelist.ModelListManager
import com.aura_on_device_ai.mnnllm.android.utils.WavFileWriter
import com.aura_on_device_ai.mnnllm.android.utils.FileUtils
import com.aura_on_device_ai.mnnllm.android.chat.chatlist.ChatListComponent
import com.aura_on_device_ai.mnnllm.android.chat.chatlist.ChatViewHolders
import com.aura_on_device_ai.mnnllm.android.chat.input.ChatInputComponent
import com.aura_on_device_ai.mnnllm.android.chat.model.ChatDataItem
import com.aura_on_device_ai.mnnllm.android.databinding.ActivityChatBinding
import com.aura_on_device_ai.mnnllm.android.llm.AudioDataListener
import com.aura_on_device_ai.mnnllm.android.llm.LlmSession
import com.aura_on_device_ai.mnnllm.android.mainsettings.MainSettings.isApiServiceEnabled
import com.aura_on_device_ai.mnnllm.android.modelsettings.SettingsBottomSheetFragment
import com.aura_on_device_ai.mnnllm.android.utils.AudioPlayService
import com.aura_on_device_ai.mnnllm.android.model.ModelTypeUtils
import com.aura_on_device_ai.mnnllm.android.model.ModelUtils
import com.aura_on_device_ai.mnnllm.android.utils.PreferenceUtils
import com.aura_on_device_ai.mnnllm.api.openai.manager.ApiServiceManager
import com.aura_on_device_ai.mnnllm.android.chat.voice.VoiceChatFragment
import com.aura_on_device_ai.mnnllm.android.chat.voice.VoiceModelsChecker
import com.aura_on_device_ai.mnnllm.android.chat.voice.VoiceModelMarketBottomSheet
...
class ChatActivity : AppCompatActivity() {
    ...
    private var captureAndExplain: Boolean = false
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        this.captureAndExplain = intent.getBooleanExtra("captureAndExplain", false)
        ...
        if (captureAndExplain) {
            startCaptureAndExplainFlow()
        }
    }

    private fun startCaptureAndExplainFlow() {
        if (!ModelTypeUtils.isVisualModel(modelId!!)) {
            return
        }
        // Ensure audio output is enabled for TTS
        binding.btnToggleAudioOutput.isSelected = true
        chatPresenter.setEnableAudioOutput(true)
        // Trigger camera capture via attachment picker
        chatInputModule?.attachmentPickerModule?.let { picker ->
            picker.toggleAttachmentVisibility()
            // Directly start camera intent if possible
            val takePhotoMethod = picker.javaClass.getDeclaredMethod("takePhoto")
            takePhotoMethod.isAccessible = true
            takePhotoMethod.invoke(picker)
        }
    }
...
//            binding.modelSwitcher.setBackgroundResource(R.drawable.bg_rounded_dropdown)
    ...
    override fun onCreateOptionsMenu(menu: Menu): Boolean {
        ...
        // Show performance metrics menu item only for MNN models
        val isMnnModel = !modelId.isNullOrEmpty() && !modelId!!.startsWith("MediaPipe/")
        val perfMetricsItem = menu.findItem(R.id.show_performance_metrics)
        perfMetricsItem.isVisible = isMnnModel
        perfMetricsItem.setChecked(
    ...
    fun onGenerateFinished(benchMarkResult: HashMap<String, Any>) {
        ...
        val recentItem = chatListComponent.recentItem
        if (recentItem == null) {
            Log.e(TAG, "onGenerateFinished: recentItem is null, cannot update UI")
            return
        }
        recentItem.loading = false
        
        // Handle error cases using a robust check
        val errorValue = benchMarkResult["error"]
        val isError = errorValue == true || (errorValue is String && errorValue.isNotEmpty())
        
        if (isError) {
            val errorMessage = (errorValue as? String) ?: (benchMarkResult["message"] as? String) ?: getString(R.string.generation_failed)
            ...
            if (!response.isNullOrEmpty() && (recentItem.text.isNullOrEmpty() || recentItem.text == "...")) {
