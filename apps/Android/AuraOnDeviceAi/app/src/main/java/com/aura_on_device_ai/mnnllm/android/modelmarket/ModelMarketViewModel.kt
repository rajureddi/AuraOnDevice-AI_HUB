...
import com.aura_on_device_ai.mls.api.download.DownloadState
import com.aura_on_device_ai.mls.api.download.ModelDownloadManager
import com.aura_on_device_ai.mnnllm.android.model.Modality
import kotlinx.coroutines.launch
import android.content.Intent
import android.os.Build
import com.aura_on_device_ai.mls.api.download.DownloadForegroundService
...
class ModelMarketViewModel(application: Application) : AndroidViewModel(application), DownloadListener {
    ...
    private fun updateDownloadInfo(modelId: String, downloadInfo: DownloadInfo) {
        ...
        // Re-apply current filter state and update market config when models change
        applyCurrentFilters()
    }
    
    private fun isDownloaded(modelId: String): Boolean {
        return downloadManager.getDownloadedFile(modelId) != null
    }
    
    private fun applyCurrentFilters() {
        var filteredList = allModels.map { wrapper ->
            // Re-check and update download complete state if needed
            if (wrapper.downloadInfo.downloadState != DownloadState.COMPLETED && isDownloaded(wrapper.modelMarketItem.modelId)) {
                 val newInfo = wrapper.downloadInfo.copy()
                 newInfo.downloadState = DownloadState.COMPLETED
                 wrapper.downloadInfo = newInfo
            }
            wrapper
        }
        ...
        filteredList = filteredList.filter { wrapper ->
            !PreferenceUtils.isModelRemoved(context, wrapper.modelMarketItem.modelId)
        }
    ...
    fun loadModels() {
        viewModelScope.launch {
            try {
                ...
                val marketItems = config.llmModels.toMutableList()
                val communityModelIds = PreferenceUtils.getCommunityModels(getApplication())
                for (id in communityModelIds) {
                    if (marketItems.none { it.modelId == id }) {
                        marketItems.add(createCommunityModelItem(id))
                    }
                }

                allModels = marketItems.map {
                    var downloadInfo = downloadManager.getDownloadInfo(it.modelId)
                    // Ensure COMPLETED state if file exists locally
                    if (downloadInfo.downloadState != DownloadState.COMPLETED && isDownloaded(it.modelId)) {
                        val newInfo = downloadInfo.copy()
                        newInfo.downloadState = DownloadState.COMPLETED
    ...
    fun deleteModel(item: ModelMarketItem) {
        viewModelScope.launch {
            ...
            if (item.vendor == "Community") {
                PreferenceUtils.removeCommunityModel(getApplication(), item.modelId)
                loadModels()
            }
        }
    }
    ...
    fun removeModel(item: ModelMarketItem) {
        PreferenceUtils.removeModelFromMarket(getApplication(), item.modelId)
        applyCurrentFilters()
    }

    private fun createCommunityModelItem(fullModelId: String): ModelMarketItem {
        val parts = fullModelId.split("/")
        val source = if (parts.size > 1) parts[0] else "ModelScope"
        val repoId = if (parts.size > 1) parts.subList(1, parts.size).joinToString("/") else fullModelId
        val name = parts.last()
        ...
        return ModelMarketItem(
            ...
            vendor = "Community",
            sizeB = 0.0,
            tags = listOf(source, "Community"),
            categories = emptyList(),
            sources = mapOf(source to repoId),
            description = "User added model from $source",
            fileSize = 0L,
            currentSource = source,
            currentRepoPath = repoId,
            modelId = fullModelId
        )
    }
...
//        }
        ...
        mainHandler.post {
            Log.d(TAG, "onDownloadProgress: modelId=$modelId, progress=${downloadInfo.progress}, stage=${downloadInfo.progressStage}")
            
            val wrapper = allModels.find { it.modelMarketItem.modelId == modelId }
            if (wrapper == null) {
                Log.d(TAG, "Model $modelId not found in allModels, refreshing list...")
                loadModels()
                return@post
            }

            if (progressStateChanged) {
                // For state changes (e.g., Preparing -> Downloading), trigger a full source-of-truth update
                updateDownloadInfo(modelId, downloadInfo)
                _itemUpdate.value = modelId
                updateServiceState()
            } else {
                // For simple progress updates, update the wrapper in-place to keep source of truth consistent
                // but only trigger the specialized progress observer to avoid full list refresh
                wrapper.downloadInfo = downloadInfo
