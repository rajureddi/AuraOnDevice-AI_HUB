...
import com.aura_on_device_ai.mnnllm.android.utils.DialogUtils
...
class MarketItemHolder(
    ...
    fun updateProgress(downloadInfo: DownloadInfo) {
        val modelMarketItem = modelMarketItemWrapper?.modelMarketItem
        val currentInfo = if (downloadInfo.downloadState != DownloadState.COMPLETED && modelMarketItem != null && modelDownloadManager.getDownloadedFile(modelMarketItem.modelId) != null) {
            val forcedInfo = downloadInfo.copy()
            forcedInfo.downloadState = DownloadState.COMPLETED
            forcedInfo
        } else {
            downloadInfo
        }
        
        Log.d(TAG, "updateProgress called for model: ${modelMarketItem?.modelId}, progress: ${currentInfo.progress}")
        
        // Update internal state
        modelMarketItemWrapper?.downloadInfo = currentInfo
        
        // Update UI
        updateDownloadState(currentInfo)
    }
    ...
    private fun updateDownloadCompleteState(downloadInfo: DownloadInfo) {
        ...
        if (modelMarketItem != null) {
            ...
            if (voiceType != MarketHolderVoiceDelegate.VoiceModelType.NONE) {
                ...
                if (modelMarketItem.isBuiltin) {
                    btnDownloadAction.text = btnDownloadAction.resources.getString(R.string.direct_use)
                } else if (downloadInfo.hasUpdate) {
    ...
    private fun updateDownloadState(downloadInfo: DownloadInfo) {
        ...
        val downloadState = currentInfo.downloadState
        ...
        when (downloadState) {
            DownloadState.NOT_START, DownloadState.FAILED -> {
                ...
                setButtonStyle()
                updateStatusText(currentInfo)
            }
            
            DownloadState.DOWNLOADING -> {
                btnDownloadAction.visibility = View.VISIBLE
                voiceDelegate.hideCheckbox()
                if (TextUtils.equals("Preparing", currentInfo.progressStage)) {
                    ...
                    val progressText = String.format("%.2f%%", currentInfo.progress * 100)
                    btnDownloadAction.text = progressText
                    setButtonStyle()
                    updateProgressText(currentInfo)
                }
            }
            ...
            DownloadState.COMPLETED -> {
                updateDownloadCompleteState(currentInfo)
            }
...
//        btnDownloadAction.strokeColor = android.content.res.ColorStateList.valueOf(primaryColor)
    ...
    private fun updateStatusText(downloadInfo: DownloadInfo) {
        ...
        tvStatus.text = when (downloadInfo.downloadState) {
            ...
            DownloadState.COMPLETED -> {
                if (modelMarketItem.isBuiltin) {
                    tvStatus.resources.getString(R.string.task_ready)
                } else if (downloadInfo.hasUpdate) {
                    tvStatus.resources.getString(
    ...
    private fun setupMenuClickListener(
        ...
        popupMenu.setOnMenuItemClickListener { item: MenuItem ->
            when (item.itemId) {
                ...
                R.id.menu_remove_model -> {
                    modelMarketItemListener.onRemoveClicked(modelMarketItemWrapper)
                }
    ...
    private fun configureMenuVisibility(popupMenu: PopupMenu, downloadState: Int) {
        ...
        // Remove option: always visible
        menu.findItem(R.id.menu_remove_model).isVisible = true
