...
package com.aura_on_device_ai.mnnllm.android.modelsettings
...
data class ModelConfig(
    ...
    companion object {
        ...
        fun loadMergedConfig(originalPath: String, overrideFilePath: String): ModelConfig? {
            return try {
                val fileOrDir = File(originalPath)
                val modelDir = if (fileOrDir.isDirectory) fileOrDir else fileOrDir.parentFile

                if (modelDir == null || !modelDir.exists()) {
                    Log.e(TAG, "Model directory not found: $originalPath")
                    return null
                }

                // Check for llm_config.json (new) or config.json (old)
                var configFile = File(modelDir, "llm_config.json")
                if (!configFile.exists()) {
                    configFile = File(modelDir, "config.json")
                }

                if (!configFile.exists()) {
                    Log.w(TAG, "No config file found in ${modelDir.absolutePath}, using default config")
                    // If no config file found, return a copy of the default config as a fallback
                    return defaultConfig.deepCopy().apply {
                        // Try to at least point to the model file if we find one
                        ...
                        if (modelFile != null) {
                            this.llmModel = modelFile.name
                        }
                    }
                }

                val originalJson = JsonParser.parseString(configFile.readText()).asJsonObject

                val overrideFile = File(overrideFilePath)
                if (overrideFile.exists()) {
                    mergeJson(originalJson, JsonParser.parseString(overrideFile.readText()).asJsonObject)
                }
                Gson().fromJson(originalJson, ModelConfig::class.java)
            } catch (e: Exception) {
                Log.e(TAG, "Failed to load config from $originalPath", e)
                null
            }
        }
        ...
        fun getDefaultConfigFile(modelId:String):String? {
            ...
            val destModelDirFile = ModelDownloadManager.getInstance(ApplicationProvider.get())
                .getDownloadedFile(modelId)
                
            destModelDirFile?.let { dir ->
                // 1. Try config.json (standard MNN-LLM)
                val configFilePath = File(dir, "config.json")
                if (configFilePath.exists()) {
                    return configFilePath.absolutePath
                }

                // 2. Try llm_config.json
                val llmConfigFilePath = File(dir, "llm_config.json")
                if (llmConfigFilePath.exists()) {
                    return llmConfigFilePath.absolutePath
                }

                // 3. Fallback for MediaPipe/single-file models:
                if (dir.isFile) {
                    val name = dir.name.lowercase(Locale.getDefault())
                    if (name.endsWith(".task") || name.endsWith(".bin") || name.endsWith(".mnn")) {
                ...
                if (dir.isDirectory) {
                    if (ModelTypeUtils.isMediaPipeModel(modelId)) {
                    ...
                    // Final fallback: if the directory exists and contains ANY .mnn or .task or .bin file, return THAT file's path
