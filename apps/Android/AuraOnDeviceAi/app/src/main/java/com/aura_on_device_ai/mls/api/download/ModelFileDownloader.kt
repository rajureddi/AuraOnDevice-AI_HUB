...
import com.aura_on_device_ai.mls.api.download.hf.HfShaVerifier
        ...
        try {
            client.newCall(request).execute().use { response ->
                ...
                if (response.code in 301..308) {
                    ...
                    if (location != null) {
                        // Safely resolve potentially relative URLs
                        theUrlToDownload = response.request.url.resolve(location)?.toString() ?: location
                    }
                }
            }
        } catch (e: IOException) {
