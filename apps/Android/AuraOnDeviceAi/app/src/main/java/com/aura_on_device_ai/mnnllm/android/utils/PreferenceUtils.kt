...
import com.aura_on_device_ai.mnnllm.android.utils.DeviceUtils.isChinese
...
object PreferenceUtils {
    ...
    // Removed/Hidden models management
    const val KEY_REMOVED_MODELS: String = "REMOVED_MODELS"
    ...
    // Removed models management methods
    fun getRemovedModels(context: Context?): Set<String> {
        val removedModelsString = getString(context, KEY_REMOVED_MODELS, "")
        return if (removedModelsString.isNullOrEmpty()) {
            emptySet()
        } else {
            removedModelsString.split(",").filter { it.isNotEmpty() }.toSet()
        }
    }

    fun removeModelFromMarket(context: Context?, modelId: String) {
        val removedModels = getRemovedModels(context).toMutableSet()
        removedModels.add(modelId)
        setString(context, KEY_REMOVED_MODELS, removedModels.joinToString(","))
    }

    fun restoreRemovedModel(context: Context?, modelId: String) {
        val removedModels = getRemovedModels(context).toMutableSet()
        removedModels.remove(modelId)
        setString(context, KEY_REMOVED_MODELS, removedModels.joinToString(","))
    }

    fun isModelRemoved(context: Context?, modelId: String): Boolean {
        return getRemovedModels(context).contains(modelId)
    }

    // Community models management
    const val KEY_COMMUNITY_MODELS: String = "COMMUNITY_MODELS"

    fun getCommunityModels(context: Context?): Set<String> {
        val modelsString = getString(context, KEY_COMMUNITY_MODELS, "")
        return if (modelsString.isNullOrEmpty()) {
            emptySet()
        } else {
            modelsString.split(",").filter { it.isNotEmpty() }.toSet()
        }
    }

    fun addCommunityModel(context: Context?, modelId: String) {
        val models = getCommunityModels(context).toMutableSet()
        models.add(modelId)
        setString(context, KEY_COMMUNITY_MODELS, models.joinToString(","))
    }

    fun removeCommunityModel(context: Context?, modelId: String) {
        val models = getCommunityModels(context).toMutableSet()
