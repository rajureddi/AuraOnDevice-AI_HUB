...
class ModelListPresenter(private val context: Context, private val view: ModelListContract.View) :
    ...
    private fun handleModelListState(state: ModelListManager.ModelListState) {
        when (state) {
            ...
            is ModelListManager.ModelListState.Success -> {
                ...
                var filteredWrappers = state.models.filter {
                    !PreferenceUtils.isModelRemoved(context, it.modelItem.modelId!!)
                }

                // Sort models
                val sortedWrappers = filteredWrappers.sortedBy { it.hasUpdate }
    ...
    override fun onItemRemoved(modelItem: ModelItem) {
        // Model removed (hidden), notify manager to refresh
        PreferenceUtils.removeModelFromMarket(context, modelItem.modelId!!)
        presenterScope.launch {
