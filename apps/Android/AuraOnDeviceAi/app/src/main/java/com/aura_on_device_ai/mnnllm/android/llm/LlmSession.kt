...
import com.aura_on_device_ai.mnnllm.android.llm.ChatService.Companion.provide
...
import com.aura_on_device_ai.mnnllm.android.modelsettings.ModelConfig.Companion.getExtraConfigFile
...
import com.aura_on_device_ai.mnnllm.android.utils.MmapUtils
import android.content.Context
import android.app.ActivityManager
import com.aura_on_device_ai.mnnllm.android.modelsettings.Jinja
import com.aura_on_device_ai.mnnllm.android.modelsettings.JinjaContext
import com.aura_on_device_ai.mnnllm.android.modelsettings.ModelConfig.Companion.loadConfig
import com.aura_on_device_ai.mnnllm.android.utils.FileSplitter
import com.aura_on_device_ai.mnnllm.android.qnn.QnnModule
class LlmSession (
    ...
    override fun load() {
        ...
        val fileOrDir = File(configPath)
        val directoryPath = if (fileOrDir.isDirectory) configPath else fileOrDir.parent ?: configPath

        checkAndMergeSplitFiles()
        
        val llmConfig = ModelConfig.loadMergedConfig(directoryPath, getExtraConfigFile(modelId))
        if (llmConfig == null) {
            Log.e(TAG, "Failed to load model config for $modelId at $directoryPath")
            modelLoading = false
            return
        }

        // Detect QNN from either config or filename/tags
        isQnn = llmConfig.backendType?.lowercase(Locale.getDefault()) == "qnn" || ModelTypeUtils.isQnnModel(modelId)
        ...
        val configMap = HashMap<String, Any>().apply {
            put("is_r1", ModelTypeUtils.isThinkingModel(modelId))
            put("is_thinking", ModelTypeUtils.isThinkingModel(modelId))
        ...
        if (isQnn && ModelTypeUtils.isVisualModel(modelId)) {
            llmConfig.visualModel = "visual_qnn_${QnnModule.modelMiddleName()}.mnn"
        }
        
        // Handle thinking model configuration
        val isThinking = ModelTypeUtils.isThinkingModel(modelId)
        if (isThinking) {
            if (llmConfig.jinja == null) {
                llmConfig.jinja = Jinja(context = JinjaContext(enableThinking = true))
            } else if (llmConfig.jinja!!.context == null) {
                llmConfig.jinja!!.context = JinjaContext(enableThinking = true)
            } else {
                llmConfig.jinja!!.context!!.enableThinking = true
            }
        }

        Log.d(TAG, "MNN_DEBUG load initNative with file: $configPath")
        ...
        if (nativePtr == 0L) {
            Log.e(TAG, "initNative failed for model: $modelId")
            modelLoading = false
            return
        }
        Log.d(TAG, "MNN_DEBUG load initNative end: $nativePtr")
        modelLoading = false
        if (releaseRequested) {
            release()
        }
    ...
    override fun generate(prompt: String,
        ...
        // Wait for model loading to finish if it's currently in progress
        var waitStart = System.currentTimeMillis()
        while (modelLoading && System.currentTimeMillis() - waitStart < 30000) { // Wait max 30s
            try {
                Thread.sleep(100)
        ...
        synchronized(this) {
            if (nativePtr == 0L) {
                Log.e(TAG, "generate failed: model not loaded (nativePtr is 0)")
                return HashMap<String, Any>().apply {
                    put("error", true)
                    put("message", "Model not loaded properly")
                }
            }
    ...
    fun runBenchmark(
        context: Context,
        commandParams: com.aura_on_device_ai.mnnllm.android.benchmark.CommandParameters,
        testInstance: com.aura_on_device_ai.mnnllm.android.benchmark.TestInstance,
        callback: com.aura_on_device_ai.mnnllm.android.benchmark.BenchmarkCallback
    ): com.aura_on_device_ai.mnnllm.android.benchmark.BenchmarkResult {
