...
package com.aura_on_device_ai.mls.api.download.hf
...
import com.aura_on_device_ai.mls.api.download.DownloadFileUtils.createSymlink
import com.aura_on_device_ai.mls.api.download.DownloadFileUtils.deleteDirectoryRecursively
import com.aura_on_device_ai.mls.api.download.DownloadFileUtils.getLastFileName
import com.aura_on_device_ai.mls.api.download.DownloadFileUtils.getPointerPathParent
...
import com.aura_on_device_ai.mls.api.download.ModelFileDownloader.FileDownloadListener
import com.aura_on_device_ai.mls.api.download.ModelRepoDownloader
import com.aura_on_device_ai.mls.api.download.hf.HfFileMetadataUtils.getFileMetadata
import com.aura_on_device_ai.mls.api.HfApiClient
import com.aura_on_device_ai.mls.api.HfApiClient.Companion.bestClient
import com.aura_on_device_ai.mls.api.HfFileMetadata
import com.aura_on_device_ai.mls.api.HfRepoInfo
...
class HfModelDownloader(override var callback: ModelRepoDownloadCallback?,
    ...
    private fun hfModelId(modelId: String): String {
        return modelId.replace("HuggingFace/", "", ignoreCase = true)
    }
    ...
    private suspend fun fetchRepoInfo(modelId: String, calculateSize: Boolean = false): HfRepoInfo {
        return withContext(DownloadCoroutineManager.downloadDispatcher) {
            runCatching {
                ...
                val response = getHfApiClient().getRepoInfoWithBlobs(hfModelId, "main", true).execute()
    ...
    override fun download(modelId: String) {
        DownloadCoroutineManager.launchDownload {
            try {
                ...
                downloadHfRepo(modelId, repoInfo)
    ...
    fun downloadHfRepo(modelId: String, hfRepoInfo: HfRepoInfo) {
        Log.d(TAG, "DownloadStart $modelId host: " + getHfApiClient().host)
        executor!!.submit {
            callback?.onDownloadTaskAdded()
            downloadHfRepoInner(modelId, hfRepoInfo)
    ...
    override suspend fun getRepoSize(modelId: String): Long {
        return withContext(DownloadCoroutineManager.downloadDispatcher) {
            runCatching {
                ...
                try {
                    val marketSize = com.aura_on_device_ai.mls.api.download.DownloadPersistentData.getMarketSizeTotal(ApplicationProvider.get(), modelId)
                    if (marketSize > 0) {
    ...
    private fun downloadHfRepoInner(modelId: String, hfRepoInfo: HfRepoInfo) {
        val hfModelId = hfModelId(modelId)
        val folderLinkFile = File(cacheRootPath, getLastFileName(hfModelId))
    ...
    private fun collectTaskList(
        ...
        for (i in hfRepoInfo.siblings.indices) {
            val subFile = hfRepoInfo.siblings[i]
            val rfilename = subFile.rfilename ?: continue
            
            // Skip unnecessary system files that often cause 403 errors on mirrors
            val lowerName = rfilename.lowercase(Locale.getDefault())
            if (lowerName == ".ds_store" || 
                lowerName.contains("__macosx") || 
                lowerName.startsWith(".git") ||
                lowerName.endsWith("/.ds_store")) {
                Log.d(TAG, "Skipping unnecessary system file: $rfilename")
                continue
            }

            metaData = metaDataList[i]!!
            val fileDownloadTask = FileDownloadTask()
            fileDownloadTask.relativePath = rfilename
            ...
            fileDownloadTask.pointerPath = File(parentPointerPath, rfilename)
    ...
    private fun requestMetaDataList(hfRepoInfo: HfRepoInfo): List<HfFileMetadata?> {
        ...
        for (subFile in hfRepoInfo.siblings) {
            val url = "https://" + getHfApiClient().host + "/" + hfRepoInfo.modelId + "/resolve/main/" + subFile.rfilename
            
            // Optimization: If the sibling already contains size and sha (from blobs=true API call), use them
            if (subFile.size > 0) {
                val metaData = HfFileMetadata()
                metaData.location = url
                metaData.size = if (subFile.lfs != null) subFile.lfs!!.size else subFile.size
                // Prefer LFS sha256 (64 chars) over git sha (40 chars) for better verification of large files
                metaData.etag = if (subFile.lfs != null) subFile.lfs!!.sha256 else subFile.sha
                
                if (!metaData.etag.isNullOrEmpty()) {
                    list.add(metaData)
                    continue
                }
            }
            
            // Fallback: Fetch metadata via HEAD request
            Log.d(TAG, "Fallback: fetching metadata for ${subFile.rfilename}")
