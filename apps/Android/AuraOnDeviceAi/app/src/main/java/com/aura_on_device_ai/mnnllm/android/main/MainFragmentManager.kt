...
import com.aura_on_device_ai.mnnllm.android.modelist.ModelListFragment
import com.aura_on_device_ai.mnnllm.android.studio.AiStudioFragment
...
class MainFragmentManager(
    ...
    private var aiStudioFragment: AiStudioFragment? = null
    ...
    fun initialize(savedInstanceState: Bundle?) {
        if (savedInstanceState == null) {
            ...
            aiStudioFragment = AiStudioFragment()
            modelMarketFragment = ModelMarketFragment()

            activity.supportFragmentManager.beginTransaction()
                .add(containerId, modelMarketFragment!!, TAG_MARKET).hide(modelMarketFragment!!)
                .add(containerId, aiStudioFragment!!, TAG_STUDIO).hide(aiStudioFragment!!)
                .add(containerId, modelListFragment!!, TAG_CHAT) //Default display
                .commit()

            activeFragment = modelListFragment
        } else {
            modelListFragment = activity.supportFragmentManager.findFragmentByTag(TAG_CHAT) as? ModelListFragment
            aiStudioFragment = activity.supportFragmentManager.findFragmentByTag(TAG_STUDIO) as? AiStudioFragment
            ...
            if (activeTag != null) {
                ...
                activeFragment = listOfNotNull(modelListFragment, aiStudioFragment, modelMarketFragment)
                    .find { !it.isHidden }
            }
        }
    ...
    private fun setupTabListener() {
        bottomNav.setOnTabSelectedListener { tab ->
            ...
            val targetFragment = when (tab) {
                BottomTabBar.Tab.CHAT -> modelListFragment
                BottomTabBar.Tab.AI_STUDIO -> aiStudioFragment
    ...
    private fun getTabForFragment(fragment: Fragment?): BottomTabBar.Tab {
        return when (fragment) {
            is ModelListFragment -> BottomTabBar.Tab.CHAT
            is ModelMarketFragment -> BottomTabBar.Tab.MODEL_MARKET
            is AiStudioFragment -> BottomTabBar.Tab.AI_STUDIO
            else -> BottomTabBar.Tab.CHAT
        }
    }
