...
import com.aura_on_device_ai.mls.api.download.DownloadPersistentData
...
import com.aura_on_device_ai.mnnllm.android.MnnLlmApplication
import com.aura_on_device_ai.mnnllm.android.modelmarket.ModelMarketItem
import com.aura_on_device_ai.mnnllm.android.modelmarket.ModelMarketCache
...
object ModelListManager {
    ...
    fun getModelTags(modelId: String): List<String> {
        ...
        // If not in immediate memory map, check current state value without blocking
        val currentState = modelListState.value
        if (currentState is ModelListState.Success) {
            val wrapper = currentState.models.find { it.modelItem.modelId == modelId }
            return wrapper?.modelItem?.getTags() ?: emptyList()
        }
    ...
    private suspend fun processModelWrapper(
        ...
        try {
            ...
            // Ensure modelName and tags are correctly extracted from market item
            val marketItem = modelItem.modelMarketItem as? com.aura_on_device_ai.mnnllm.android.modelmarket.ModelMarketItem
            if (marketItem != null) {
                modelItem.modelName = marketItem.modelName
                modelItem.tags = marketItem.tags
            } else if (modelItem.modelName.isNullOrEmpty()) {
                modelItem.modelName = ModelUtils.getModelName(downloadedModel.modelId)
            }

            // Consistent tag logic
...
//            localModel.loadMarketTags(context)
            ...
            val marketItemForTags = localModel.modelMarketItem as? com.aura_on_device_ai.mnnllm.android.modelmarket.ModelMarketItem
            ...
            if (localModel.modelName.isNullOrEmpty()) {
                val marketItem = localModel.modelMarketItem as? com.aura_on_device_ai.mnnllm.android.modelmarket.ModelMarketItem
    ...
    private fun createModelIdFromPath(context: Context, absolutePath: String): String? {
        ...
        val relativePath = absolutePath.substring(mnnModelsPath.length + 1).replace('\\', '/')
        return when {
            ...
            relativePath.contains("taobao-mnn/") -> {
                 "HuggingFace/$relativePath"
            }
