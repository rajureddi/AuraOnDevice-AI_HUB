...
//#define USE_DEBUG_PROMOT
    ...
    std::string debug_prompt = "<audio>/data/user/0/com.aura_on_device_ai.mnnllm.android/files/history/1746690738111/record_1746690751335.wav</audio>";
    ...
    if (llm_ != nullptr && callback != nullptr) {
        ...
        llm_->setWavformCallback([this, callback = std::move(callback)](const float *ptr, size_t size, bool last_chunk) {
            ...
            if (last_chunk) {
                ...
                MNN::AUDIO::save("/data/data/com.aura_on_device_ai.mnnllm.android/files/output.wav", waveform_var, 24000);
                waveform.clear();
            }
#endif
